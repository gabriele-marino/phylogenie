...
def test_operate_with_invalid_sizes():
    with pytest.raises(ValueError):
        SkylineVector([1, 2]) * SkylineVector([3, 4, 5])  # pyright: ignore
    with pytest.raises(ValueError):
        SkylineVector([1, 2]) + SkylineVector([3])  # pyright: ignore
    with pytest.raises(ValueError):
        SkylineVector([1, 2]) - SkylineVector([])  # pyright: ignore
    with pytest.raises(ValueError):
        SkylineVector([1, 2]) / SkylineVector([3, 4, 5, 6])  # pyright: ignore
